...
working_directory =r"H:\PEAKS_export_DIA_Quant_20220824"
output_dir = r"D:\DIA\20220824\Quant_Results\LogP30"
...
for w in folder_search:
    ...
    prot_pept4 =  prot_pept4[prot_pept4['-10lgP'] >= 30]
    index_append = []
    ...
    #fractions_w_dups = grouped_final_results['Fraction'].values.tolist()
    #fractions = []
    ...
    #frac_1 = fractions[0]
    #frac_2 = fractions[1]
    #frac_1_filtered = grouped_final_results[grouped_final_results['Fraction'] == frac_1]
    #frac_1_filtered_final = frac_1_filtered.drop_duplicates(subset=('Peptide','Scan','Absolute Mod Location','PTMs formatted','Protein Group','Protein ID', 'Protein Accession'))
    #frac_1_filtered_final = frac_1_filtered.drop_duplicates(subset=('Protein Group','Absolute Mod Location','PTMs formatted'))
    #frac_2_filtered = grouped_final_results[grouped_final_results['Fraction'] == frac_2]
    #frac_2_filtered_final = frac_2_filtered.drop_duplicates(subset=('Peptide','Scan','Absolute Mod Location','PTMs formatted','Protein Group','Protein ID', 'Protein Accession'))
    #frac_2_filtered_final = frac_2_filtered.drop_duplicates(subset=('Protein Group','Absolute Mod Location','PTMs formatted'))
    total_filtered_final = grouped_final_results.drop_duplicates(subset=('Peptide','Scan','Absolute Mod Location','PTMs formatted','Protein Group','Protein ID', 'Protein Accession'))
    #total_filtered_final = grouped_final_results.drop_duplicates(subset=('Protein Group','Absolute Mod Location','PTMs formatted'))

    #out_path = output_dir + '\\' + w + '_Q1.csv'
    #with open(out_path,'w',newline='') as filec:
    #                        writerc = csv.writer(filec)
    #                        frac_1_filtered_final.to_csv(filec,index=False)

    #out_path = output_dir + '\\' + w + '_Q2.csv'
    #with open(out_path,'w',newline='') as filec:
    #                        writerc = csv.writer(filec)
